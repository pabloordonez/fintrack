...
export class MessageBusService
{
    ...
    {
        if (!this.handlers.has(token.type))
        ...
        for (const messageHandler of messageHandlers)
            handler.splice(handler.indexOf(messageHandler), 1);

        /* if the handler doesn't have more message handlers, we can delete it */
        if (handler.length === 0)
            this.handlers.delete(token.type);
    }
    ...
    async send<T>(message: T, token?: RegistrationToken<T>): Promise<void>
    {
        ...
        if (!(message as any).constructor)
            throw new Error('Message does not have a constructor, and the system can not infer the type.');

        if (!this.handlers.has((message as any).constructor as ObjectType<T>))
            return;
